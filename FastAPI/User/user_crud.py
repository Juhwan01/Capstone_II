...
from core.models import UserModel, RequestModel, Transaction, IngredientModel
...
class UserCRUD:
    ...
    async def make_transaction(self, payload: TransDTO):
        ...
        result = await self._session.execute(
            select(RequestModel).filter_by(id=payload.request_id)
        )
    ...
    async def close_transaction(self, trans_id: int):
        ...
        if transaction:
            ...
            request_result = await self._session.execute(select(RequestModel).filter_by(id=re_id))
